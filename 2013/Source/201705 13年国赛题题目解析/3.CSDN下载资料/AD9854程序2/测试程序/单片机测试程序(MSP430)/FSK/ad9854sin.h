...
{                           
  ...
  _NOP();
  send_byte(MOD,0X03);    //Triangle=0，QDAC为内部输入，UFSK模式，内部刷新时钟
  ...
  send_byte(PD,0x10);         //turn on I DAC and DIG,but still off Q DAC,PD: CR,DC,DC,COMP,  0,QDAC,DAC,DIG
}                               //DAC开了IDAC就开了，给1就关
//**********发送1字节数据************//
void send_byte(uchar add,uchar data)
{
...
void ad9854f2(double f) 
{
  ...
  table9854[4]=(uchar)((((((f*256)/160000000-table9854[0])*256-table9854[1])*256-table9854[2])*256-table9854[3])*256);
  table9854[5]=(uchar)(((((((f*256)/160000000-table9854[0])*256-table9854[1])*256-table9854[2])*256-table9854[3])*256-table9854[4])*256);
  send_byte(FTW2_6,table9854[0]);     //M=f*2^48/160 算出来的M转为2进制
  send_byte(FTW2_5,table9854[1]);   
  send_byte(FTW2_4,table9854[2]);
  send_byte(FTW2_3,table9854[3]);
  send_byte(FTW2_2,table9854[4]);
  send_byte(FTW2_1,table9854[5]);
}
